(ns reserve.operation
  "OperationActor -- one central-banking operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (ReserveOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Central Bank Reserve Governor
  ...
  One graph run = one central-banking operation (intake -> advise ->
  ...
  decision to a human operator (central-bank staff). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/open-reserve-account`/`:actuation/release-settlement-
  batch` ALWAYS reach this node when the governor is clean -- see
  `reserve.phase`."
...
(defn build
  ...
  `reserve.store/Store`).
  opts:
    :advisor      -- a `reserve.reserveadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (reserveadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ReserveOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (reserveadvisor/-advise advisor store request)]
            {:proposal p :audit [(reserveadvisor/trace request p)]})))

      ;; Central Bank Reserve Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
