(ns reserve.portable-cljs-test-runner
  ...
  `reserve.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c reserve.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [reserve.corporate-intel-test]
            [reserve.facts-test]
            [reserve.governor-contract-test]
            [reserve.kernels.gate-test]
            [reserve.phase-test]
            [reserve.registry-test]
            [reserve.store-contract-test]))
